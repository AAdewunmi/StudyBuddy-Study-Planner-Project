...
def test_insights_dashboard_shows_user_owned_insights(client) -> None:
    ...
    insight = StudyInsightFactory(summary="Django testing improves review.")
    client.force_login(insight.owner)
...
def test_insights_dashboard_hides_other_users_insights(client) -> None:
    ...
    StudyInsightFactory(summary="Hidden user insight", owner=other_user)

    client.force_login(own_insight.owner)
    response = client.get(reverse("insights:list"))
...
def test_insights_dashboard_displays_empty_state(client) -> None:
    ...
    assert response.status_code == 200
    assert b"No study insights yet" in response.content